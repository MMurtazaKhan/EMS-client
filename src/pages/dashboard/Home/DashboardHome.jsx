...
const DashboardHome = () => {
  ...
  const fetchReviewsCount = async () => {
    const response = await axios.get(
      `https://fiesta-flex.vercel.app/review/${user?.email}`
    );
  ...
  const fetchProductsOrderCount = async () => {
    const response = await axios.get(
      `https://fiesta-flex.vercel.app/product-orders/${user?.email}`
    );
    if (response.status === 200) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
      ...
      {widgets.totalPayment && (
        <Widget
          icon={<MdOutlinePayment className="text-amber-700" size={30} />}
          title="Total Payment"
          count={totalBookings_Payment}
        />
      )}
